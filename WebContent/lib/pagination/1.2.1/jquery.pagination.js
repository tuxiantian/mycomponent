...
jQuery.fn.pagination = function(maxentries, opts){
	...
	return this.each(function() {
		...
		// 跳转至某一页
		if (opts.gotoPage) { // 修改。zw。
			$("#"+opts.gotoPage).siblings("a.ui-button").unbind("click").bind("click",function(e){
				gotoPage(e);
			})

			$("#"+opts.gotoPage).unbind("keyup").bind("keyup",function(e){
				var _self = $(this);
			...
			function gotoPage(ev){
				var newPageIndex = parseInt($("#"+opts.gotoPage).val()-1, 10);
				var totalPage = numPages();
				if(newPageIndex>=totalPage || newPageIndex<0){
					if(typeof result_tips == 'function'){
						result_tips("请输入正确的页码！", 1);
					}else{
						alert("请输入正确的页码！");
					}
					return false;
				}else{
					pageSelected(newPageIndex, ev);
					$("#"+opts.gotoPage).val("");
				}
			}
		}
